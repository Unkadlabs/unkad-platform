// SINGLE IMPLEMENTATION of Somali sentence segmentation.
//
// Plain JavaScript on purpose. The app imports it through lib/sentences.ts and
// the operational scripts import it directly with node, so there is one
// definition of what a sentence is rather than one per caller.
//
// That mattered: status.mjs, milestone.mjs and corpus-data.mjs each carried
// their own inline copy of an older splitter, so the public counters, the
// milestone watcher and the published dataset were quietly reporting three
// different sentence counts for the same corpus. Anything that needs to count
// sentences imports this file. Nothing reimplements it.
//
...
const unmask = (s) => s.split(MASK).join('.');
...
function isBoundaryBreak(before, after) {
...
function isGlossaryLine(s) {
  if (/[.!?؟۔]$/.test(s)) return false;
  const m = s.match(/^([^=]+)=([^=]+)$/);
  if (!m) return false;
  const words = (part) => part.trim().split(/\s+/).filter(Boolean).length;
...
export function isUsableSentence(s) {
...
export function countSentences(raw) {
